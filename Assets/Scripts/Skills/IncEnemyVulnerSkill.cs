...
public class IncEnemyVulnerSkill : ComboSkill {
    ...
    public override void AIUseSkill(ActionParams args) {
        Debug.Log("IncEnemyVulnerSkill used!");
    ...
    private void ArgumentCheck(ActionParams args) {
        ...
        if (args == null)
            throw new ArgumentNullException("IncEnemyVulnerSkill received null argument list from program");
        else if (args.Count() < 2)
            throw new ArgumentException("IncEnemyVulnerSkill received argument list with length " + skillParams.Count() + "; Expecting arguments length 2");
        else if (args.GetParamType(0) != typeof(double))
            throw new ArgumentException("IncEnemyVulnerSkill received argument of invalid type " + skillParams.GetParamType(0) + " at argument 0; Expecting type double");
        else if (args.GetParamType(1) != typeof(int))
            throw new ArgumentException("IncEnemyVulnerSkill received argument of invalid type " + skillParams.GetParamType(1) + " at argument 1; Expecting type int");
        // If ActionParams has more than 1 argument returned, warn and ignore
        else if (args.Count() != 2) {
            Debug.LogWarning("IncEnemyVulnerSkill received more than 2 argument; Make sure this is the desired data");
        }
    }
}
